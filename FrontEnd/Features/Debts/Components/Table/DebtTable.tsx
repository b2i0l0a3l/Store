...
const columns: Column<Debt>[] = [
  { key: "clientName", label: "Name" },
  {
    ...
    render: (item) => {
      ...
      return (
        <span
          className={`inline-flex items-center gap-1 px-3 py-1 rounded-full text-xs font-semibold ${isPaid ? "bg-emerald-500/15 text-emerald-400 border border-emerald-500/25" : "bg-rose-500/15 text-rose-400 border border-rose-500/25"}`}
        >
          <span
            className={`w-1.5 h-1.5 rounded-full ${isPaid ? "bg-emerald-400" : "bg-rose-400"}`}
          />
