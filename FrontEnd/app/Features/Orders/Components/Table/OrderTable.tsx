...
const columns: Column<order>[] = [
  ...
  },
  {
    ...
    label: "Order Type",
    render: (item) => (
      <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold ${ORDER_TYPE_STYLES[item.orderType] ?? "bg-slate-700/50 text-slate-300 border border-slate-600/50"}`}>
        {item.orderType}
      </span>
    ),
  },
  {
    ...
    label: "Order Status",
    render: (item) => (
      <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold ${ORDER_STATUS_STYLES[item.orderStatus] ?? "bg-slate-700/50 text-slate-300 border border-slate-600/50"}`}>
        <span className={`w-1.5 h-1.5 rounded-full mr-1.5 ${item.orderStatus === "Completed" ? "bg-emerald-400" : item.orderStatus === "Pending" ? "bg-amber-400" : item.orderStatus === "Cancelled" ? "bg-rose-400" : "bg-slate-400"}`} />
        {item.orderStatus}
      </span>
    ),
  },
