...
export async function getProducts(pageNumber: number = 1, pageSize: number = 10,Category?:string,Barcode?:string,ProductName?:string) {
    try {
       ...
       if (ProductName) uri.set("ProductName", ProductName);
        const url = `http://localhost:5107/api/v1/Product/GetAll?${uri.toString()}`
        ...
        const nextPage = r.value.pageNumber < r.value.totalPages ? r.value.pageNumber + 1 : undefined;
